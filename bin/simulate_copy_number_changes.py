...
def get_commandline_options():
    ...
    parser.add_argument("--subsample_somatic_snvs", dest="subsample_somatic_snvs",
                        help="Subsample percentage for clonal somatic SNV truth file", type=float, default=1)
    ...
    if not options.ploidy_depth:
        ...
        parser.print_help()
        sys.exit(2)
    if (not options.somatic_snv_files and options.clonal_percs) or (
                options.somatic_snv_files and not options.clonal_percs):
...
def read_in_truth_set(truth_file):
    ...
    for line in truth_set:
        ...
        truth_set_cn_calls[chr].append(
            {'start': int(start), 'end': int(end), 'firsthaplotype_CN': CN_hapA, 'secondhaplotype_CN': CN_hapB})
    print("ts ", truth_set_cn_calls)
    return truth_set_cn_calls


def write_eligible_read(read_line, region, paired_read, subsample_fraction, mutate_somatic_variants, output_bam,
                        somatic_variants, haplotype):
    # debugging variables
    global cd, r1, xx, r2
    ...
    if read_line.pos < read_line.pnext:  # skipping reads with pair same start point for the mo
        # below line causes random reads to not be added
        # the smaller the subsample_fraction, the more reads are skipped
        # if True:
        if random.random() < subsample_fraction:
            if mutate_somatic_variants:
                somatic_variants_in_read = somatic_variants.find(read_line.pos,
                                                                 read_line.pos + read_line.inferred_length)
                read_line = add_in_somatic_variants(read_line, haplotype,
                                                    somatic_variants_in_read)  # CHANGE READS IN PLACE
            if read_line.is_proper_pair:
                r1 += 1
                if read_line.qname not in paired_read:
...
def write_eligible_reads(read_container, region, paired_read, mutate_somatic_variants, output_bam, min_element,
                         somatic_variants):
...
def create_synthetic_bam(bam_file_haplotype1, bam_file_haplotype2, output_bam_file, query_chr, mutate_somatic_variants,
                         somatic_variants, truth_set_cn_calls, ploidy_depth, bam_depth):
    ...
    for region in truth_set_cn_calls[query_chr]:
        ...
        for h in hs:
            ...
            if subsample_fraction > 1:
                sys.exit("Oversampling error: CN=%d, ploidy_depth=%d, bam_depth=%d, subsample_fraction=%f" % (
                    region[h], ploidy_depth, bam_depth, subsample_fraction))
            print("region " + str(region), "ploidy_depth ", str(ploidy_depth), " copy_number ", region[h],
        ...
        for iterator in izip_longest(samfile_haplotype1.fetch(query_chr, region['start'], region['end']),
                                     samfile_haplotype2.fetch(query_chr, region['start'], region['end']),
                                     fillvalue=dummy_read):
            read_lines = list(iterator)
            # case1: first bam shorter   
            if read_lines[0].pos == -1:
                min_element = read_lines[1].pos
                write_eligible_read(read_lines[0], region, paired_read, subsample_fractions[0], mutate_somatic_variants,
                                    output_bam, somatic_variants, hs[0])
            # case2: second bam shorter                    
            elif read_lines[1].pos == -1:
                min_element = read_lines[0].pos
                write_eligible_read(read_lines[1], region, paired_read, subsample_fractions[1], mutate_somatic_variants,
                                    output_bam, somatic_variants, hs[1])
            # case3: reads from both bams
                ...
                if reserve_min_element >= min_element:
                    ...
                    if read_lines[1].pos == read_lines[0].pos:
                        write_eligible_read(read_lines[0], region, paired_read, subsample_fractions[0],
                                            mutate_somatic_variants, output_bam, somatic_variants, hs[0])
                        write_eligible_read(read_lines[1], region, paired_read, subsample_fractions[1],
                                            mutate_somatic_variants, output_bam, somatic_variants, hs[1])
                    ...
                    write_eligible_reads(reserve_read_container, region, paired_read, mutate_somatic_variants,
                                         output_bam, min_element, somatic_variants)
...
def what_freq_for_haplotype_cn(copy_number):
    ...
    if copy_number == 1:
        ...
        if random_value < 0.25:
            freq = 0.33
        if 0.25 <= random_value <= 0.75:
            freq = 0.66
        if random_value > 0.75:
            freq = 1

    if freq is not None:
        print("copyn ", copy_number, " freq ", freq)
    return freq


def read_in_somatic_vcf_file(somatic_snv_files, clonal_percs, query_chr, truth_set_cn_calls, output_dir,
                             subsample_somatic_snvs):
    ...
    for (somatic_snv_file, clonal_perc) in zip(somatic_snv_files, clonal_percs):
        ...
        for line in fh:
            ...
            # just using first tier reads for now
            # (first, second, third)=sorted([int(cv.split(',')[0] ) for cv in l], reverse=True)

            if random.random() > 0.5:
                somatic_haplotype_cn = 'firsthaplotype_CN'
            else:
                somatic_haplotype_cn = 'secondhaplotype_CN'

            print("pos ", pos, "shcn ", somatic_haplotype_cn, " r ")

            region_cn = 2
            for region in truth_set_cn_calls[query_chr]:
                if region['start'] <= pos <= region['end']:
                    region_cn = region[somatic_haplotype_cn]
            somatic_mutation_freq = float(assign_freq_based_on_ploidy(region_cn))
            somatic_mutation_freq *= float(clonal_perc)

            # theoretically bug: could have snv and indel at same pos #also bug if snp or indel last/first on read
            h.add(pos, pos, {'pos': pos, 'ref': ref, 'alt': alt, 'line': line, 'freq': somatic_mutation_freq,
                             'somatic_haplotype': somatic_haplotype_cn})

            h2[pos] = {'pos': pos, 'ref': ref, 'alt': alt, 'line': line, 'freq': somatic_mutation_freq,
                       'somatic_haplotype': somatic_haplotype_cn}
...
def add_in_somatic_variants(read_line, which_haplotype, somatic_variants_in_read):
    """Add SNV variants
    Number of ploidy; at the mo max ploidy per haplotype is 3;
    but what are the prob of the somatic snvs being on 1,2 or 3 ploidy:
    ...
    for d in somatic_variants_in_read:
        if d['somatic_haplotype'] == 'both' or d['somatic_haplotype'] == which_haplotype:
            ...
            if d['somatic_haplotype'] == which_haplotype and random_no < d['freq']:
                ...
                q = read_line.qual
                if re.search(',', d['alt']):
                    # ignore tri-allelic positions for now
                    continue
                ...
                if up_to == -1:
                    up_to = 0  # to fix bug
                read_line.seq = read_line.seq[:up_to] + d['alt'] + read_line.seq[up_to + 1:]
                read_line.qual = q
